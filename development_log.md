...
### 2025-05-30 背景图片资源优化与清理
- 优化背景图片预加载逻辑，仅在随机选中后加载所需背景，避免无用流量
- 移除CSS默认背景，防止页面初始加载时自动请求背景图片
- 删除images目录下所有未被使用的原始背景图片文件，进一步精简资源
- 以上优化显著提升了页面加载性能和用户体验
### 2025-05-30 广告弹窗结构与适配优化
- 广告弹窗将原<video>标签改为<div>容器，支持webp动图播放
- 移除#ad-video的内联style，所有宽高与居中由CSS和JS动态控制
- 优化弹窗在移动端的自适应显示，防止图片溢出和错位
- 提升了广告展示的兼容性和用户体验
### 2025-05-30
- 新增：生成并应用favicon.png，网页在浏览器标签栏显示红色R字图标。
